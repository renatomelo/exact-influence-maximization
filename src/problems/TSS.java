...
public abstract class TSS {
	...
	/**
	 * Determine the threshold of each vertex in the graph. thr(v)= inDegree(v)/2
	 * 
	 * @param vSet
	 *            set of vertex
	...
	/**
	 * Determine the threshold of each vertex in the graph. thr(v)= inDegree(v)/4
	 * 
	 * @param vSet
	 *            set of vertex
	 * @return a vector of threshold
	 */
	public int[] smallThreshold(Set<Vertex> vSet) {
		int[] thr = new int[vSet.size()];
		...
		for (Vertex v : vSet) {
			if (g.inDegreeOf(v) == 0) {
				...
				thr[v.getIndex()] = (int) Math.ceil((double) g.inDegreeOf(v) / 4);
			}
			v.setThreshold(thr[v.getIndex()]);
		}
		return thr;
	...
	/**
	 * Determine the threshold of each vertex in the graph. thr(v)= 3*inDegree(v)/4
	 * 
	 * @param vSet
	 *            set of vertex
	 * @return a vector of threshold
	 */
	public int[] largeThreshold(Set<Vertex> vSet) {
		int[] thr = new int[vSet.size()];
		...
		for (Vertex v : vSet) {
			if (g.inDegreeOf(v) == 0) {
				...
				thr[v.getIndex()] = (int) Math.ceil((double) g.inDegreeOf(v) * 3 / 4);
			}
			v.setThreshold(thr[v.getIndex()]);
		}
		return thr;
